...
class CAH(commands.Cog):
    ...
    async def removeMember(self, user, game = None):
        ...
        for member in game['Members']:
            ...
            if removed['IsBot']:
                ...
                msg = f'***{removed['User'].display_name}*** **left the game - reorganizing...**'
            ...
            if judgeChanged:
                ...
                if newJudge['IsBot']:
                    ...
                    if newJudge == member:
                        ...
                        msg += f'\n\n***{newJudge['User'].display_name}*** **is now judging!**'
            await member['User'].send(msg)
        return game
            ...
            if user:
                ...
                if user['IsBot']:
                    ...
                    if not member == user:
                        ...
                        msg = f'*{user['User'].display_name}* submitted their {card}! '
            if submitted < totalUsers:
    ...
    async def winningCard(self, ctx, game, card):
        ...
        if winner['By']['IsBot']:
            ...
            winnerName = winner['By']['User'].display_name
        for member in game['Members']:
            if member['IsBot']:
                continue
    ...
    async def showPlay(self, ctx, user):
        ...
        if game['Members'][game['Judge']]['User'] == user:
            ...
            if game['Members'][game['Judge']]['IsBot']:
                ...
                judge = f'*{game['Members'][game['Judge']]['User'].display_name}* is'
        ...
        for member in game['Members']:
            ...
            if member['Points'] >= self.winAfter:
                ...
                if member['IsBot']:
                    ...
                    stat_embed.set_author(name=f'{member['User'].display_name} is the WINNER!!')
    ...
    async def game(self, ctx, *, message = None):
        ...
        userGame = self.userGame(ctx.author)
        if not userGame:
    ...
    async def say(self, ctx, *, message = None):
        ...
        # msg = '*{}* says: {}'.format(ctx.author.name, message)
        member_count = 0
    ...
    async def newcah(self, ctx):
        ...
        await ctx.send('**You created game id:** ***{}***'.format(gameID))
    ...
    async def joincah(self, ctx, *, id = None):
        ...
        for member in game['Members']:
            if member['IsBot']:
                continue
            await member['User'].send(f'***{ctx.author.display_name}*** **joined the game!**')
    ...
    async def score(self, ctx):
        ...
        for user in users:
            ...
            if user['Points'] == 1:
                if user['User']:
                    # Person
                    msg += f"{i}. *{user['User'].display_name}* - 1 point\n"
                else:
                    # Bot
                ...
                if user['User']:
                    # Person
                    msg += f"{i}. *{user['User'].display_name}* - {user['Points']} points\n"
                else:
                    # Bot
    ...
    async def laid(self, ctx):
        ...
        for user in users:
            ...
            if user['Laid']:
                if user['User']:
                    # Person
                    msg += f"{i}. *{user['User']}* - Cards are in.\n"
                else:
                    # Bot
                    msg += '{}. *{} ({})* - Cards are in.\n'.format(i, user.get("Name", self.botName), user['ID'])
            else:
                if user['User']:
                    # Person
                    msg += f"{i}. *{user['User']}* - Waiting for cards...\n"
                else:
                    # Bot
